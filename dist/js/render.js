...
//     localStorage.setItem("version", 250);
...
function renderComment(data) {
	let idUser = parseInt(getCookie("id"));
	document.querySelector(".comments-block").appendChild(
		cDiv(null,
		[
			cDiv(
			{class:`direct-chat-msg ${(idUser== data.ID) ? "right" : ""}`}, 
				[
					cDiv(
						{class:"direct-chat-info clearfix"},
						[
							cEl("span", [{class:`direct-chat-name pull-${(idUser == data.ID) ? "right" : "left"}`}], data.first_name),
						]
					),
					cEl("img", [{class:"direct-chat-img"},{src:data.photo}]),
					cEl("p", [{class:"direct-chat-text"}], data.text),
				]	
			)
	]));
}
function renderComments() {
	...
	containerFluid.children[0].children[1].appendChild(
		cDiv(
			{class:"card bg-light direct-chat-success"},
			[
				cDiv(
					{class:"bcard-header text-muted"},
					cEl("h4", null,"Сообщения")
				),
				cDiv(
					[{class:"card-body pt-0 comments-block"},{style:"height: 350px; overflow-x: scroll;"}]
				),
				cDiv(
					{class:"card-footer"},
					[
						cDiv(
							...
							[
								cEl("input", [{type:"text"}, {class:"form-control send-comment"},{placeholder:"..."}]),
								cEl("button", [{type:"button"}, {class:"btn btn-info btn-sm"}, {onclick:"sendComment()"}], "Отправить")
							]
						)
					]
				),
...
containerFluid.children[0].children[0].className = "col-md-8 order-2 order-md-1";
containerFluid.children[0].children[1].className = "col-md-4 order-1 order-md-2";
  getJSON("/api/news/getAll", null, data => {
    remove(".render-loader");
    i = 0;
    data.forEach(dataElem => {
      containerFluid.children[0].children[0].appendChild(
        cDiv(
          { class: "callout" },
...
function renderEditorEvent() {
  containerFluid.appendChild(
    cDiv({ class: "row" }, [
      ...
      cDiv({ class: "col-md-3 order-1 order-md-2 " }, [printAdvice(), renderDescEvent()])
    ])
  );
}
