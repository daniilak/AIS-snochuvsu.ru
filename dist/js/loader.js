...
function renderSelect2(s) {
  ...
  $(s.class)
    .val(JSON.parse(s.r))
    .trigger("change");
  $(s.class).on("change", function() {
    val = $(s.class).select2("val");
    console.log( $($(s.class)[0]).data("id"))
    console.log($($(s.class)[0]).data("typeselect"));
    if ($($(s.class)[0]).data("typeselect") == "supervisors") {
      let sendingData = new FormData();
      sendingData.append("id", $($(s.class)[0]).data("id"));
