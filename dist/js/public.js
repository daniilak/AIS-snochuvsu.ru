...
function sendZ() {
	...
	for (i = 0; i < memberID; i++) {
		if (document.querySelector(`[data-mem-val="${i}"][data-mem-type="inst"]`)) {
			if (document.querySelector(`[data-mem-val="${i}"][data-mem-type="lN"]`).value.length == 0) {
				alert("Не все заполнены поля")
				return
			}
	    	members.push({
				inst: document.querySelector(`[data-mem-val="${i}"][data-mem-type="inst"]`).value,
				lN: document.querySelector(`[data-mem-val="${i}"][data-mem-type="lN"]`).value,
				num: document.querySelector(`[data-mem-val="${i}"][data-mem-type="num"]`).value,
				fN: document.querySelector(`[data-mem-val="${i}"][data-mem-type="fN"]`).value,
				mN: document.querySelector(`[data-mem-val="${i}"][data-mem-type="mN"]`).value,
	...
	if (members.length == 0 || name_project.length == 0) {
		alert("Пустая заявка"); return;
	}
	let leaders = [];
	for (i = 0; i < leaderID; i++) {
		if (document.querySelector(`[data-leader-val="${i}"][data-leader-type="lN"]`)) {
			if (document.querySelector(`[data-leader-val="${i}"][data-leader-type="lN"]`).value.length == 0) {
				alert("Не все заполнены поля"); return;
			}
			if (document.querySelector(`[data-leader-val="${i}"][data-leader-type="fN"]`).value.length == 0) {
				alert("Не все заполнены поля"); return;
			}
			...
			members.push({
				...
				pos: document.querySelector(`[data-leader-val="${i}"][data-leader-type="pos"]`).value,
	    	});
		}
	}
	let sendingData = new FormData();
	...
	getJSON("/api/requests/sendRequest", sendingData, data=>{
		if (data.answer == 0) {
			alert("Заявка успешно отправлена, модератор мероприятия скоро её промодерирует!")
			$("#openSend").modal("hide");
		}
		if (data.answer == 1) {
			alert("Заявка не отправлена. Участника нет в системе lk.chuvsu.ru по указанным данным: (фамилия, №зачетки)!")
			$("#openSend").modal("hide");
		}
	});
}
